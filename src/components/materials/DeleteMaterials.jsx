...
import { materialsDelete } from "../../slices/sliceMaterials";

export default function DeleteMaterials() {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const { mId } = useParams();
    const { deleteStatus } = useSelector((state) => state.materials);

    const handleDelete = async () => {
        dispatch(materialsDelete(mId));
        navigate("/materials")
    }
    return (
                ...
                <div className="flex p-4 w-[100%] h-[100%]">
                    <div className="flex flex-col items-center justify-center h-[100%] w-[100%]">
                        ...
                        <div className="flex w-[100%] h-[50px] mt-[15px]">
                            <button onClick={() => navigate("/materials")} className="w-[100%] text-white bg-blue-700 hover:bg-blue-600 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-md px-5 py-2.5 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                                Batal
                            </button>
                        </div>
                    </div>
